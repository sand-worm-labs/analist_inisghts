...
DATA_DIR = Path("data")
OUTPUT_DIR = Path("analysis") / "gap_analysis"
...
def plot_histogram(diffs: np.ndarray, bins: int = 50, save_path: Path = None):
    ...
    plt.title("Distribution of Gaps Between Query IDs", fontsize=14, fontweight='bold')
...
def analyze_gaps(ids: List[int], total_range: int, prefix: str = "analysis") -> Dict:
    ...
    stats = {
        ...
        "gaps_over_1000": int(np.sum(diffs > 1000)),
    ...
    print(f"[INFO] Gaps > 1000: {stats['gaps_over_1000']:,}")
